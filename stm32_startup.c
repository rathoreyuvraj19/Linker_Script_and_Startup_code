/* ================= Cortex-M4 + STM32F4 Complete Vector Table ================= */
...
void Reset_Handler(void)
{
    ...
    uint32_t size = &_data_end - &_data_start;
    uint8_t *pDest = (uint8_t *)&_data_start; // RAM location of .data
    uint8_t *pSrc = (uint8_t *)&_text_end;    // Flash location of .data
    ...
    /* Step 2:  Initialize .bss section */
    uint32_t size_bss = &_bss_end - &_bss_start;
    pDest = (uint8_t *)&_bss_start; // RAM location of .bss
    ...
    // Trap if main returns - embedded applications typically should not return from main
    while (1)
        ;
}